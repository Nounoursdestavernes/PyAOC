...
def check_day_structure(day_number: int = 1) -> None:
    ...
    if not os.path.exists(dir_name):
        return sys.exit(f"Directory {dir_name} does not exist")
    
    os.chdir(dir_name)

    if not os.path.exists("inputs"):
        os.chdir("..")
        return sys.exit(f"Directory {dir_name}/inputs does not exist")

    os.chdir("inputs")
    if not os.path.exists("input.txt"):
        os.chdir("..") # Go back to day folder
        os.chdir("..") # Go back to src folder
        return sys.exit(f"File {dir_name}/inputs/input.txt does not exist")
    
    os.chdir("..")

    if not os.path.exists("part1.py"):
        os.chdir("..")
        return sys.exit("File part1.py does not exist")    

    if not os.path.exists("part2.py"):
        os.chdir("..")
        return sys.exit("File part2.py does not exist")    

    if not os.path.exists("benchmark"):
        os.chdir("..") # Go back to src folder
        return sys.exit(f"Folder {dir_name}/benchmark does not exist")

    if not os.path.exists(os.path.join("benchmark", "benchmark.txt")):
        os.chdir("..") # Go back to src folder
        return sys.exit(f"File {dir_name}/benchmark/benchmark.txt does not exist")
...
def check_name_folder(name: str) -> None:
    ...
    if day_number < 1 or day_number > 25:
        return sys.exit("Invalid name : day_number must be between 1 and 25")
